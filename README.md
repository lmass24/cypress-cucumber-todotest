...
- Se diseñaron los escenarios de prueba en formato tipo GHERKIN tratando de que estos fueran lo más genéricos posibles, representando comportamientos y reglas de negocio (como lo sugiere la metodología BDD) y no tanto escenarios con pasos técnicos y detallados como se harían en las metodologías tradicionales. Sin embargo, en algunos casos se violaron estas "reglas" para demostrar el funcionamiento de algunas funcionalidades puntuales como el uso de las tablas o los valores de entrada leídos desde el .feature desde la clase step definitions. En conclusión, los escenarios pueden ser muchos mas genéricos todavía. 
- Los datos de prueba de mayor relevancia se almacenaron mediante la funcionalidad de los fixtures que tiene cypress que es un archivo json desde donde se consumen dichos datos de prueba. Para los datos mas confidenciales como las credenciales y contraseñas para los nuevos usuarios se almacenaron como variables de entorno en el archivo cypress.config.js. 
- Timeouts:
    En esta página la mayoría de los elementos se renderizan por completo lo que hace que las pruebas no sean complejas, sin embargo existía un caso especifico en donde los elementos no se renderizaban por completo y cypress no los encontraba bajo ningun tipo de contexto, por lo que se hizo uso de estrategias no tan bien vistas como los tiempos de espera ramdoms (cy.waits), aunque solo fue en un solo caso. Por precaución también se instaló la librería cypress-waitfor que lo que hace es esperar a que X elemento exista y sea visible para proceder con los siguientes pasos del tests.
    NOTA: Cypress recomienda interceptar la ruta de dominio en la que estamos trabajando, ponerle un determinado alias y que espere a que esas rutas o peticiones esten resueltas, esta estrategia funciona muy bien sobre todo al momento de emplear alguna regresión y queremos agregar robustés, sin embargo, no soluciona el problema de la renderización que comenté anteriormente. De igual manera por la facilidad de las pruebas para este caso no fué necesario hacer uso de esta estrategia de espera. 

## Errores o mejoras en la aplicación:

- La activación de la cuenta por medio del correo funciona, pero si se deja pasar las 24 horas que se indica en la página para hacer el proceso de activación, todavía funciona dicha activación.
...
- Límite de caracteres o dígitos permitidos. Si bien los campos tienen esta validación ya incluída, se deberían mostrar mensajes que indiquen los límites o caracteres máximos permitidos (como en el caso de la contraseña). NOTA: este mensaje se mostraría solo en caso de superar estps límites.
...
- Mejorar el tema de reportes, para este caso no vi necesario agregar alguna librería de reporting sin embargo recomiendo algunas como Allure y mochawesome sobretodo al momento de implemenetar CI/CD.
